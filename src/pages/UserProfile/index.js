...
export default function UserProfile() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      resetForm({ values: { fullName: values.fullName, email: values.email, phone: values.phone, gender: values.gender } });
  ...
  const handleUpdateProfile = (values) => {
    ...
    fetch(
      `https://64acf61eb470006a5ec514b7.mockapi.io/movie/account/`+localStorage.getItem("id"),
      {
        method: "PUT", // or PATCH
        headers: { "content-type": "application/json" },
